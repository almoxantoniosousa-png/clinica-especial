...
export function RoleSidebar({ userRole }: RoleSidebarProps) {
          ...
          {menu.map((item) => {
            ...
            return (
              ...
              >
                ...
                {ativo && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-blue-600"/>}
              </Link>
            );
          })}
        </nav>
  ...
  return (
    <>
      <aside className="sidebar-desktop bg-white border-r border-slate-200 min-h-screen flex-col">
        <SidebarContent />
      </aside>

      <div className="topbar-mobile fixed top-0 left-0 right-0 z-40 bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between shadow-sm">
      ...
      <div className="spacer-mobile h-16"/>

      {menuAberto && (
        <>
          <div
            className="overlay-mobile fixed inset-0 z-40 bg-black/40 backdrop-blur-sm"
            onClick={() => setMenuAberto(false)}
          />

          <div className="topbar-mobile fixed top-0 left-0 h-full w-72 z-50 bg-white shadow-xl flex flex-col">
