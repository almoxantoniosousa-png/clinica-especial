...
-- Nome real da policy em produção é "protocolos_gerenciar" (a leitura já é
-- liberada à parte, pela policy "protocolos_leitura").
drop policy if exists "protocolos_gerenciar" on public.protocolos_conduta;
create policy "protocolos_gerenciar" on public.protocolos_conduta
  for all using (meu_role() in ('adm', 'admin', 'gestao'));
