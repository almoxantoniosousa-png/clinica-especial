...
export default function GestaoCriancasPage() {
  ...
  const filtradas = criancas.filter(c => {
    ...
    const matchStatus =
      ...
      filtroStatus === "ativo" ? c.ativo !== false :
      c.ativo === false;
    return matchBusca && matchStatus;
  });

  const totalAtivo = criancas.filter(c => c.ativo !== false).length;
  const totalInativo = criancas.filter(c => c.ativo === false).length;
  ...
  function abrirWhatsApp(telefone: string, nome: string) {
    ...
    const msg = encodeURIComponent(`Olá! Somos da Clínica Abraço ABA. Entramos em contato referente ao atendimento de ${nome}.`);
    window.open(`https://wa.me/55${numero}?text=${msg}`, "_blank");
  }

  function abrirEmail(email: string, nome: string) {
    window.open(`mailto:${email}?subject=Clínica Abraço ABA - ${nome}`, "_blank");
  }
  ...
  const statusOpcoes: { value: FiltroStatus; label: string; count: number }[] = [
    { value: "ativo",   label: "Ativos",   count: totalAtivo },
    { value: "inativo", label: "Inativos", count: totalInativo },
    { value: "todos",   label: "Todos",    count: criancas.length },
  ];
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por nome ou responsável..." value={busca}
            onChange={e => setBusca(e.target.value)}
            className="w-full h-10 pl-9 pr-4 rounded-xl border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        </div>
        <div className="flex gap-1 bg-white border border-slate-200 rounded-xl p-1">
          {statusOpcoes.map(op => (
            <button key={op.value} onClick={() => setFiltroStatus(op.value)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition ${
                filtroStatus === op.value ? "bg-blue-900 text-white" : "text-slate-500 hover:bg-slate-50"
              }`}>
              {op.label}
              <span className={`text-xs px-1.5 rounded-full ${filtroStatus === op.value ? "bg-white/20" : "bg-slate-100 text-slate-600"}`}>
                {op.count}
              </span>
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-2xl border border-slate-200">
          ...
          <p className="text-sm text-slate-400 mt-2">Nenhuma criança encontrada.</p>
        </div>
      ) : (
        <>
              ...
              <div key={c.id} className={`bg-white border rounded-2xl p-4 shadow-sm space-y-3 ${c.ativo === false ? "opacity-60" : ""}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        c.ativo === false ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-700"
                      }`}>
                ...
                <div className="flex gap-2">
                  {c.telefone_responsavel ? (
                    ...
                    <div className="flex-1 h-9 flex items-center justify-center text-xs text-slate-300 rounded-xl border border-slate-100 bg-slate-50">Sem tel.</div>
                  )}

                  {c.email_responsavel ? (
                    <button onClick={() => abrirEmail(c.email_responsavel, c.nome)}
                    ...
                    <div className="flex-1 h-9 flex items-center justify-center text-xs text-slate-300 rounded-xl border border-slate-100 bg-slate-50">Sem email</div>
                  )}

                  {c.instagram && (
                    <button onClick={() => abrirInstagram(c.instagram)}
                      className="h-9 w-9 flex items-center justify-center bg-pink-50 hover:bg-pink-100 text-pink-600 rounded-xl border border-pink-200 transition active:scale-95 shrink-0">
                      <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
