            ...
            {exibidos.map(esp => (
              <div key={esp.id} className="bg-white rounded-xl border border-slate-200 p-4">
                <div className="flex items-start gap-3">
                  <div className={`w-11 h-11 shrink-0 flex items-center justify-center rounded-full font-bold text-sm ${corAvatar(esp.nome)}`}>
                    {iniciais(esp.nome)}
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex gap-2 mt-3">
                      {esp.whatsapp ? (
                        <button onClick={() => abrirWhatsApp(esp.whatsapp, esp.nome)}
                      ...
                      {esp.email ? (
                        <button onClick={() => abrirEmail(esp.email)}
