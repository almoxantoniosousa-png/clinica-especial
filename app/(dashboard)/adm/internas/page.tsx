...
export default function InternasPage() {
  ...
  async function salvar() {
    ...
    if (editando) {
      ...
      else { mostrarFeedback("sucesso", "Colaboradora atualizada!"); carregarColaboradoras(); }
    } else {
      const { error } = await supabase.from("colaboradoras_internas").insert(payload);
      if (error) mostrarFeedback("erro", error.message);
      else { mostrarFeedback("sucesso", "Colaboradora cadastrada!"); carregarColaboradoras(); }
    }
    setSalvando(false);
  ...
  async function deletar(id: string) {
    ...
    if (error) mostrarFeedback("erro", error.message);
    else { mostrarFeedback("sucesso", "Colaboradora removida."); carregarColaboradoras(); }
    setDeletandoId(null);
  }
  ...
  const CardColaboradora = ({ col }: { col: any }) => (
    <div className="bg-white rounded-xl border border-slate-200 p-4 hover:border-blue-200 transition">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => setDeletandoId(col.id)}
  ...
  const Secao = ({ titulo, items, cor }: { titulo: string; items: any[]; cor: string }) => (
    items.length > 0 ? (
      <div>
    ...
    <div className="space-y-6 pb-10">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          <Secao titulo="Auxiliar Administrativa" items={auxAdm} cor="bg-purple-100 text-purple-700" />
          <Secao titulo="Agente de Limpeza" items={limpeza} cor="bg-emerald-100 text-emerald-700" />
        </div>
      )}
